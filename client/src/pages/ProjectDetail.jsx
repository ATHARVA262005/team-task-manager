...
import { Plus, UserPlus, Trash2, X, Calendar } from 'lucide-react';
...
export default function ProjectDetail() {
  ...
  const handleDragEnd = (result) => {
    ...
    const canMove = isAdmin || task.assigneeId === user?.id;
    if (!canMove) return;

    const newStatus = destination.droppableId;
    if (newStatus !== task.status) {
      handleUpdateTask(task.id, { status: newStatus }, task.version);
    }
  };
                    ...
                    >
                      {tasks.map((task, index) => (
                        <Draggable
                          ...
                          draggableId={task.id}
                          index={index}
                          isDragDisabled={!isAdmin && task.assigneeId !== user?.id}
                        >
                          {(provided, snapshot) => (
                            <div
                              ref={provided.innerRef}
                      ...
                      {tasks.length === 0 && !snapshot.isDraggingOver && (
                        <div className="text-center py-8 text-sm text-gray-400">
                          {isAdmin ? 'Drag tasks here or create new ones' : 'No tasks'}
                        </div>
                      )}
                    </div>
                  )}
...
function TaskCard({ task, isAdmin, currentUserId, onEdit, onDelete, onStatusChange, members, isDragging }) {
  ...
  return (
    <div className={clsx(
      'bg-white rounded-lg border p-4 shadow-sm transition',
      isDragging ? 'shadow-lg border-indigo-300 rotate-2' : 'hover:shadow-md',
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        {isAdmin && (
          <div className="flex gap-1 flex-shrink-0">
            <button onClick={(e) => { e.stopPropagation(); onEdit(task); }} className="p-1 text-gray-400 hover:text-indigo-600 transition">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
              </svg>
            </button>
            <button onClick={(e) => { e.stopPropagation(); onDelete(task.id); }} className="p-1 text-gray-400 hover:text-red-500 transition">
